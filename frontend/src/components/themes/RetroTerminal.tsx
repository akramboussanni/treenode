  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uUseMouse: { value: mouseEffectsEnabled ? 1 : 0 },
    ...
    const update = (t: number) => {
      ...
      const elapsed = (t * 0.001 + timeOffsetRef.current) * 0.5;
    ...
    if (mouseEffectsEnabled) ctn.addEventListener("mousemove", handleMouseMove);

    return () => {
      cancelAnimationFrame(rafRef.current);
      resizeObserver.disconnect();
      if (mouseEffectsEnabled) ctn.removeEventListener("mousemove", handleMouseMove);
